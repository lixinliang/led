		...
		switchShape : function ( n, fast, differOnly ) {
			...
			if (n.dots.length > this.dots.length) {
				...
				for (var d = 1; d <= size; d++) {
					this.dots.push(new S.Dot(a.w / 2, a.h / 2, config.pointRadius, config.pointColor));
				}
			}
			var d = 0;
			var i = 0;
			while (n.dots.length > 0) {
				...
				if (this.dots[d].s) {
					this.dots[d].move(new S.Point({
						z : Math.random() * 20 + config.pointRadius*2,
						a : Math.random(),
						h : 18
					}));
				} else {
			...
			for (var i = d; i < this.dots.length; i++) {
				if (this.dots[i].s) {
					...
					this.dots[i].move(new S.Point({
						...
						z : Math.random() * config.pointRadius*0.8,
						h : 0
					}));
				}
			}
